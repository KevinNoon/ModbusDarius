...
public class OutputStreamASCII extends OutputStreamSerial {
    ...
    public void write(byte[] bytes, int offset, int length) throws IOException {
        ...
        byte[] ascii = DataUtils.toAscii(bytes, offset, length).getBytes(StandardCharsets.US_ASCII);
        super.write(ascii);
    }
    ...
    public void write(int b) throws IOException {
        lrc += (byte) b;
        byte[] bytes = DataUtils.toAscii((byte) b).getBytes(StandardCharsets.US_ASCII);
        super.write(bytes);
    }

    private void writeChecksum() throws IOException {
        byte[] bytes = DataUtils.toAscii((byte) -lrc).getBytes(StandardCharsets.US_ASCII);
        super.write(bytes);
    }
